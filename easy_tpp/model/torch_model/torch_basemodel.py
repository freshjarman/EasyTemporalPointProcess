...
class TorchBaseModel(nn.Module):
    ...
    def compute_loglikelihood(self, time_delta_seq, lambda_at_event,
                              lambdas_loss_samples, seq_mask, type_seq):
        ...
        event_ll = -F.nll_loss(
            ...
            target=type_seq,
            ignore_index=self.
            pad_token_id,  # Padded events have a pad_token_id as a value
            reduction=
            'none',  # Does not aggregate, and replaces what would have been the log(marked intensity) with 0.
        )
        ...
        if self.use_mc_samples:
            non_event_ll = total_sampled_lambdas.mean(
                dim=-1) * time_delta_seq * seq_mask
        else:  # Use trapezoid rule
            non_event_ll = 0.5 * (total_sampled_lambdas[..., 1:] +
                                  total_sampled_lambdas[..., :-1]).mean(
                                      dim=-1) * time_delta_seq * seq_mask
    ...
    def predict_one_step_at_every_event(self, batch):
        ...
        time_seq, time_delta_seq, event_seq = (time_seq[:, :-1],
                                               time_delta_seq[:, :-1],
                                               event_seq[:, :-1])
        ...
        # [batch_size, seq_len-1, num_sample]
        ...
        # [batch_size, seq_len-1, num_sample, num_marks]
        ...
        intensities_weighted = torch.einsum('...s,...sm->...m', weights,
        ...
        # [batch_size, seq_len-1]
        dtimes_pred = torch.sum(accepted_dtimes * weights,
                                dim=-1)  # compute the expected next event time
