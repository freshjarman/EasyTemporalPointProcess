...
class EventSampler(nn.Module):
    ...
    def __init__(self, num_sample, num_exp, over_sample_rate,
                 num_samples_boundary, dtime_max, patience_counter, device):
    ...
    def compute_intensity_upper_bound(self, time_seq, time_delta_seq,
        ...
        Args:
            ...
            compute_last_step_only (bool): wheter to compute the last time step only.
        ...
        dtime_for_bound_sampled = time_delta_seq[:, :,
                                                 None] * time_for_bound_sampled
        ...
        # [batch_size, seq_len]
        bounds = intensities_for_bound.sum(dim=-1).max(
            dim=-1)[0] * self.over_sample_rate
    ...
    def sample_accept(self, unif_numbers, sample_rate, total_intensities,
        ...
        criterion = unif_numbers * sample_rate[:, :, None,
                                               None] / total_intensities
    ...
    def draw_next_time_one_step(self,
        ...
        intensity_upper_bound = self.compute_intensity_upper_bound(
            time_seq, time_delta_seq, event_seq, intensity_fn,
            compute_last_step_only)
        ...
        total_intensities = torch.tile(total_intensities[:, :, None, :],
                                       [1, 1, self.num_sample, 1])

        # [batch_size, seq_len, num_sample, num_exp]
        exp_numbers = torch.tile(exp_numbers[:, :, None, :],
                                 [1, 1, self.num_sample, 1])
        ...
        res = self.sample_accept(unif_numbers, intensity_upper_bound,
                                 total_intensities, exp_numbers)
